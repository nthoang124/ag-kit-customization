...
- **`rag-engineer`**: Thiết kế Advanced RAG pipeline, cấu hình vector databases, re-ranking và hybrid search.
- **`prompt-engineer`**: Thiết kế prompt có cấu trúc (few-shot, chain-of-thought) tối ưu token và quản lý chi phí.
- **`langgraph`**: Cấu trúc kiến trúc Multi-Agent phối hợp tĩnh dựa trên state machines với LangGraph.
- **Evaluation & Reliability (Evals)**: Thiết lập pipeline đánh giá tự động (LLM-as-a-judge), regression tests.
- **MLOps & Serving**: Observability, tracing, caching và quản lý cost trong môi trường production.
...
@Data_AI/ai-engineer Thiết kế Advanced RAG pipeline cho legal document search:
