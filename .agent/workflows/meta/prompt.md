...
risk: none
source: self
required_skills: [Data_AI/prompt-orchestrator]
...
- User đã chỉ định skill (`@Development/backend-developer`) → Gọi trực tiếp.
...
2. Với mỗi skill (priority High/Medium):
   - `view_file` đọc `.agent/skills/{category}/{skill-name}/SKILL.md` (chủ động tìm thư mục phân loại như Development, Architecture...)
