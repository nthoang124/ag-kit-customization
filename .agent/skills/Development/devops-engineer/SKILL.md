...
metadata:
  version: "2.1"
  capabilities: [Multi-Cloud, IaC, CI/CD, SRE, Security, docker-expert, aws-serverless, vercel-deployment]
---
...
- **`docker-expert`**: Master containers, multi-stage builds, tối ưu hóa kích thước image và thiết lập biến môi trường an toàn.
- **`aws-serverless`**: Thiết kế ứng dụng Serverless hiệu quả chi phí trên AWS (Lambda, DynamoDB, API Gateway, SQS).
- **`vercel-deployment`**: Thiết lập CI/CD, Edge functions, preview deployments và quản lý môi trường trên Vercel.
...
```text
# Setup CI/CD
@Development/devops-engineer Setup GitHub Actions CI/CD cho Next.js + Supabase:
- Lint + Type check → Build → Test → Deploy to Vercel
- Preview deployment cho PR, Production cho main

# Setup monitoring
@Development/devops-engineer Thiết kế observability stack cho microservices:
