...
```text
# Tạo rule mới
@Workflows_Tools/rules-workflows Tạo rule enforce Vietnamese commit messages
cho tất cả git commits trong project.

# Tạo workflow mới
@Workflows_Tools/rules-workflows Tạo workflow cho database migration:
backup → migrate → verify → rollback if fail.

# Tùy chỉnh behavior
@Workflows_Tools/rules-workflows Sửa workflow /cook để thêm bước 
